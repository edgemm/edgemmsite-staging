...
<!--Begin smc news cat cloud-->
<?php
if (is_desc_cat(65) ) {
  	$categorycheck = get_category( get_query_var( 'cat' ) );
	$cat_id = $categorycheck->cat_ID;
	$categories =  get_categories('child_of=65');        
?>
<ul id="filterNavSMC" class="clearfix">
<?php if ($cat_id == 65) {$classis2 = "current-cat";} else {$classis2 = "";} ?>
	<li class="allBtn"><a href="/category/marketing-news/" class="<?php echo $classis2; ?>">All</a></li>
<?php 
	foreach  ($categories as $category)
	{
	if ($cat_id == $category->cat_ID) {$classis = "current-cat";} else {$classis = "";}
	echo '<li class="allBtn"><a class="'.$classis.'" href="/category/marketing-news/'.$category->slug.'/">'.$category->name.'</a></li>';
	}
}
?>
</ul>			
<!--End smc news cat cloud-->
