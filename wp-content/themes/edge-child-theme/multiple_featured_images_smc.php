...
function MultiPostThumbnailsSetAsThumbnail(thumb_id, id, post_type, nonce){
	...
	jQuery.post(ajaxurl, {
		...
		public static function get_the_post_thumbnail_url2($post_type, $thumb_id, $post_id = NULL, $size = 'post-thumbnail', $attr = '' , $link_to_original = false) {
			global $id;
			$post_id = (NULL === $post_id) ? $id : $post_id;
			$post_thumbnail_id = self::get_post_thumbnail_id($post_type, $thumb_id, $post_id);
			$img_src = wp_get_attachment_image_src($post_thumbnail_id, "background-blur-smc");
			$img_url = $img_src[0];		
			return $img_url;			
		}
